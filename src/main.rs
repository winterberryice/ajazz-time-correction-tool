...
/// Performs the full sequence of SET and GET commands with the CURRENT system time.
fn perform_full_handshake(device: &HidDevice) -> Result<(), Box<dyn Error>> {
    ...
    // --- Stage 3: The actual Time Sync command with CURRENT time ---
    println!("  - Using current system time...");
    let now = Local::now();
    let mut cmd3 = [0u8; 64];
    ...
    println!("\nSUCCESS: The full time-sync handshake was sent to the keyboard!");
